...
int main( void ) {
  ...
  while ( !glfwWindowShouldClose( window ) ) {
    ...
    glUseProgram( shader_program ); // Put the shader program, and the VAO, in focus in OpenGL's state machine.
    glBindVertexArray( vao );
  
    glDrawArrays( GL_TRIANGLES, 0, 3 ); // Draw points 0-3 from the currently bound VAO with current in-use shader.
    
    glfwSwapBuffers( window ); // Put the stuff we've been drawing onto the visible area.
  }
